...
if [[ ! "$BUILD_IMAGE" =~ ^rust:1\.96\.1-bookworm@sha256:[0-9a-f]{64}$ ]]; then
  echo "Linux release build image must pin rust:1.96.1-bookworm by digest" >&2
  exit 2
fi
...
docker run --rm --pull never \
  ...
  bash -c '
    ...
    if [[ "$(rustc --version)" != rustc\ 1.96.1* ]]; then
      echo "release container did not provide the pinned Rust 1.96.1 toolchain" >&2
      rustc --version >&2
      exit 1
    fi
