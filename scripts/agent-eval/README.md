...
| `EVAL_NO_DELEGATE` | `0` | `0` = natural agent behavior, including any Explore sub-agent delegation encouraged by the host instructions. This is the representative mode because that is how Cartograph is actually used and where its value shows. `1` appends a single-agent steering prompt to both arms so every tool call is observable. That is useful for debugging and A/B steering, but it suppresses the delegation pattern Cartograph is built around, so it is not for publishing. |
...
> The top-level `README.md` no longer publishes this agent-behavior benchmark table. If these results are republished later, use `fullCalls` / `fullCgCalls` so the table reflects main-thread plus recovered sub-agent tool calls.
