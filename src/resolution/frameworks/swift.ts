  ...
  detect(context: ResolutionContext): boolean {
    ...
    for (const file of allFiles) {
      if (file.endsWith('.swift')) {
        ...
        if (
          content &&
          (content.includes('import UIKit') || content.includes('UIViewController') || content.includes('UIView'))
        ) {
          return true;
        }
      }
  ...
  resolve(ref: UnresolvedRef, context: ResolutionContext): ResolvedRef | null {
    ...
    if (ref.referenceName.endsWith('View') && !ref.referenceName.endsWith('ViewController')) {
      const result = resolveByNameAndKind({
        name: ref.referenceName,
        kinds: CLASS_KINDS,
        preferredDirPatterns: UIVIEW_DIRS,
        context,
      });
      if (result) {
        return {
    ...
    if (ref.referenceName.endsWith('Cell')) {
      const result = resolveByNameAndKind({
        name: ref.referenceName,
        kinds: CLASS_KINDS,
        preferredDirPatterns: CELL_DIRS,
        context,
      });
      if (result) {
        return {
...
export const vaporResolver: FrameworkResolver = {
  ...
  resolve(ref: UnresolvedRef, context: ResolutionContext): ResolvedRef | null {
    ...
    if (ref.referenceName.endsWith('Controller')) {
      const result = resolveByNameAndKind({
        name: ref.referenceName,
        kinds: VAPOR_CONTROLLER_KINDS,
        preferredDirPatterns: VAPOR_CONTROLLER_DIRS,
        context,
      });
      if (result) {
        return {
    ...
    if (/^[A-Z][a-zA-Z]+$/.test(ref.referenceName)) {
      const result = resolveByNameAndKind({
        name: ref.referenceName,
        kinds: CLASS_KINDS,
        preferredDirPatterns: FLUENT_MODEL_DIRS,
        context,
      });
      if (result) {
        return {
    ...
    if (ref.referenceName.endsWith('Middleware')) {
      const result = resolveByNameAndKind({
        name: ref.referenceName,
        kinds: VAPOR_CONTROLLER_KINDS,
        preferredDirPatterns: VAPOR_MIDDLEWARE_DIRS,
        context,
      });
      if (result) {
        return {
  ...
  extractNodes(filePath: string, content: string): Node[] {
    ...
    const groupedRoutePattern =
      /\.grouped\s*\(\s*["']([^"']+)["']\s*\)\s*\.(get|post|put|patch|delete)\s*\(\s*["']([^"']+)["']/g;
