...
export const expressResolver: FrameworkResolver = {
  ...
  extractNodes(filePath: string, content: string, getStripped?: () => string): Node[] {
    ...
    const routePatterns = [/(app|router)\.(get|post|put|patch|delete|all|use)\(\s*['"]([^'"]+)['"]/g];
...
function resolveControllerMethod(controller: string, method: string, context: ResolutionContext): string | null {
  ...
  const methodNodes = methodCandidates.filter(
    (n) =>
      (n.kind === 'method' || n.kind === 'function') && n.filePath.toLowerCase().includes(controller.toLowerCase()),
  );
 ...
 */
function resolveServiceMethod(serviceName: string, method: string, context: ResolutionContext): string | null {
