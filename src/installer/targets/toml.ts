 ...
 * single dotted-key table block (`[mcp_servers.cartograph]`) inside an
 ...
 *
 * Strategy: treat the file as text. Find the `[mcp_servers.cartograph]`
 ...
 *     the full dotted key `[mcp_servers.cartograph]`).
 ...
 */
function serializeTomlTableBody(values: Record<string, string | string[]>): string {
  const lines: string[] = [];
