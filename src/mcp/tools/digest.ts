...
export function buildDigestHotspotsSpec(
  ...
  return {
    ...
    emptyState: [
      '## 🔥 Hotspots',
      '_No hotspot data — needs git history + centrality. Run `cartograph index` for a first index, or `cartograph admin index` to rebuild._',
      '',
    ].join('\n'),
  };
}
...
export const DIGEST_BIOMARKER_STATE_NOTES: Record<Exclude<BiomarkerSectionState, 'populated'>, string> = {
  'never-ran':
    '_No biomarker data — run `cartograph index` for a first index, or `cartograph admin index` to rebuild._',
