...
function formatNoHotspotsMessage(cg: ReturnType<ToolCtx['getCartograph']>): string {
  ...
  if (state === null) {
    ...
    lines.push(
      ...
      '- The index has not been built yet — run `cartograph index`.',
  ...
  lines.push(
    ...
    '- The index has not finished building — run `cartograph index` for first setup or `cartograph admin index` to rebuild.',
