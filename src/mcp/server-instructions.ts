...
- **MCP load context** — the advertised tool list itself costs context before any call runs. Operators can start focused servers with \`--no-write-tools\` and repeated \`--disable-tool <name>\`; agents can only reduce per-call output with \`lowTokens\` after connection.
