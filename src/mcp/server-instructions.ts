...
- **Maximum token savings** — pass \`lowTokens: true\` on supported high-volume tools (\`find\`, \`graph\`, \`context\`, \`explore\`, \`at_range\`, \`node\`, \`files\`, \`imports\`) to apply compact rows, field projection, lower caps, or source suppression in one switch.
- **MCP load context** — the advertised tool list itself costs context before any call runs. Operators can start focused servers with \`--profile core|read-only|review\`, \`--no-write-tools\`, and repeated \`--disable-tool <name>\`; agents can only reduce per-call output with \`lowTokens\` after connection.
