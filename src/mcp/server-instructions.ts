...
- **MCP load context** — the advertised tool list itself costs context before any call runs. Operators can measure it with \`cartograph mcp-budget\` or \`bun run measure:mcp-load\`, then start focused servers with \`--profile core|read-only|review\`, \`--no-write-tools\`, and repeated \`--disable-tool <name>\`; \`--low-tokens-default\` reduces per-call output after connection but not \`tools/list\`.
