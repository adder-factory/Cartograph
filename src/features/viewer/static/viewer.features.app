...
const SAVED_VIEWS_KEY = viewerProjectStorageKey('cartograph-viewer-saved-views-v1');
const GRAPH_SNAPSHOT_KEY = viewerProjectStorageKey('cartograph-viewer-graph-snapshot-v1');
...
function viewerLocalStateKeys() {
  ...
  try {
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (key?.startsWith(viewerProjectStoragePrefix())) keys.add(key);
      else if (key?.startsWith('cartograph-viewer-') && !key.startsWith('cartograph-viewer-project:')) keys.add(key);
    }
  } catch {}
