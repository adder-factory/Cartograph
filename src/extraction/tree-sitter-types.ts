...
export interface ExtractorContext {
  ...
  /** Walk a subtree for type references (type_identifier, language-
   *  specific wrappers like Python `type`, C# `generic_name`, etc.)
   *  and emit edges from `fromNodeId`. Used by language-specific
   *  extractors that don't dispatch through extractFunction (e.g.
   *  ReScript's let-binding path, Pascal procedure heads). */
...
export interface LanguageExtractor {
  ...
  /**
   * F#65 — override symbol name extraction for languages whose name
   * isn't a simple field lookup. ObjC multi-keyword selector
   * reconstruction (`doThing:with:` from `- (void)doThing:(id)x with:(id)y`)
   * uses this. Return undefined to fall through to the default
   * field-based lookup.
   */
  resolveName?: (node: SyntaxNode, source: string) => string | undefined;
  /**
   * F#65 — extract property name when the generic name walk fails.
   * ObjC `@property` declarations wrap their name inside a struct
   * declarator chain that the default walker doesn't follow.
   */
