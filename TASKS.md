...
- [x] Include `projectPath` in MCP `llm-plan` apply instructions.
  - Repro: `cartograph_admin({action: "llm-plan", projectPath: "~/projects/cartograph"})` ends with `cartograph_admin({action: "llm-apply", preset: "<id>"})`, but `cartograph_admin({action: "llm-apply", preset: "skip"})` fails because `projectPath` is required.
