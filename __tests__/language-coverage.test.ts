 ...
 * one edge. One `it` row per language, so a regression names the
 * language that broke.
 *
 ...
 *
 * Scope: one `fixture.<ext>` per language — pure LANGUAGE-extraction
 * coverage. Schema-DSL recognizers (Zod, Pydantic, …) have their own
 ...
 *  they exist purely so framework resolvers can dispatch on them
 *  (yaml: F#62, so the Drupal resolver can pre-load
 *  tree-sitter-yaml and walk routing.yml on its own). The fixture
 *  still gets a parse-without-error check, but the symbol-floor +
 *  edge-floor assertions are skipped because requiring them would
 *  force a feature-creep extractor change. */
const KNOWN_ZERO_SYMBOL_LANGUAGES: ReadonlySet<string> = new Set(['yaml']);
...
describe('language-coverage parity-guard', () => {
  ...
  // A vacuous pass guard: if discovery silently returns nothing,
  // `it.each([])` registers zero tests and the suite passes for free.
